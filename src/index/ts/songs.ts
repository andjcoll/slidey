...
function searchSongs() {
  const input: any = document.getElementById('search-songs');
...
for (const child of Array.from(document.querySelector(".navbar ul").children)) {
  ...
  child.addEventListener("click", function (e: MouseEvent) {
    hideAllActions();
...
document.addEventListener('paste', function (e) {
  ...
  e.preventDefault();
  const frozenClipboard: { [key: string]: any } = {
...
document.addEventListener("click", function (e: PointerEvent) {
  ...
  if (e.target != copyBibleButton && !copyBibleButton.contains(e.target as Node)) {
    copyBibleButton.classList.remove("selected");
  }
});

let dataToCopy: { [key: string]: any };
async function createSongsList() {
  ...
  for (let song of songs) {
    ...
    li.addEventListener("click", async function (e: MouseEvent) {
      const deleteButton = li.querySelector(".delete-song");
      if (e.target == deleteButton ||
        Array.from(deleteButton.querySelectorAll("*")).includes(e.target as Element)) return;

      const songData = await getSong(song);
    ...
    li.addEventListener("copy", function (e: ClipboardEvent) {
      e.preventDefault();
